...
const ATTACK_RANGE = 190  # Дистанция атаки
const ATTACK_COOLDOWN = 1.5  # Задержка между атаками
...
func take_damage(damage, attack_direction: Vector2 = Vector2.ZERO):
	if can_take_damage == true:
		...
		if health <= 0:
			global.coins += int(23 * global.current_coins_mult)
			self.queue_free()
...
func attack():
	if can_attack:
		...
		await get_tree().create_timer(shot_arrow_delay).timeout
